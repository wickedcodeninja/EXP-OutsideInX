...
-- Bijection between N and all valid identifiers
...
-- Ordering is induced by the bijection of {RV, UV} with Int
...
data TyConName = TyConName 
  { runTyConName :: String 
  } deriving (Eq, Ord)
  
data TyFamName = TyFamName 
  { runTyFamName :: String 
  } deriving (Eq, Ord)

data TyClassName = TyClassName 
  { runTyClassName :: String 
  } deriving (Eq, Ord)
  
data Monotype 
  = TyVar TypeVariable
  | TyCon TyConName [Monotype]
  | TyFam TyFamName [Monotype]
  deriving (Eq)

-- (Hopefully?) Total Order on Monotypes. Used to ensure termination of the simplification process. 
-- Simplifications only apply to one of the two possible orientations of a given type equality and
-- a chain of simplifications never results in a constraint of the same size with opposite orientation,
-- thus avoiding infinite rewrites.
...
-- C-Constraints generated by the OutsideIn algorithm. 
data Implication 
  = Implication (Set Constraint) (Set ExtendedConstraint) Touchables
  deriving (Eq, Ord)

data ExtendedConstraint 
  = BaseConstraint        Constraint  -- A constraint specific to the parameter X in OutsideIn(X)
  | ImplicationConstraint Implication -- A constraint specific to the OutsideIn solver
  deriving (Eq, Ord)
  
-- Q-Constraints for a specific instantiation of X in the OutsideIn(X) framework
data Constraint 
  = Equality Equality
  | TypeClass TyClassName [Monotype]
  deriving (Eq, Ord)
...
data Toplevel 
  = TopTyFam TyFamName [([Monotype], Monotype)]        -- F V ~ v. List of ordered equations for closed type families.                                      
  | TopClass TyClassName (Set Constraint) [Monotype]   -- Q => D V
...
solver tl given tch wanted = do
  ...
  let simplifiedWanted = Set.map (applySubst subst) (implicationPart wanted)
      ...
      implicationSolver (Implication q_i c_i tch_i) = do
  ...
  _ <- traverse_ implicationSolver simplifiedWanted
...
-- `pick n xs` gives all length n subsequences of xs
...
-- Find the first Type Family in a list of arguments and return the family, together
-- with a function to reconstruct the original argument list with the family replaced
-- by something else.
